    ...
    const Vertex *const pVertex, VertexFeatureInfoMap &vertexFeatureInfoMap) const
{
    ...
    std::cout << "Energy deposition" << std::endl;
    ...
    if (!m_legacyVariables)
    {
        ...
        for (const auto& [key, value] : kdTreeMap) std::cout << "key: " << key << std::endl; 

        vertexEnergy = this->GetVertexEnergy(pVertex, kdTreeMap);
        std::cout << "    Vertex energy: " << vertexEnergy << std::endl;
    }

    std::cout << "vertex feature info" << std::endl;
